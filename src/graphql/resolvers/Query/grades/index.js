/**
 * =======================================================================================
 *  DATALOADER-ENABLED RESOLVER FILE (GRADE)
 * =======================================================================================
 *
 * This file contains the GraphQL resolvers for the Grade type and the
 * DataLoader factory (`createLoaders`) to optimize database queries.
 *
 * It includes pagination for nested lists like 'subjects'. Pagination arguments
 ...
 * 1. Run a GraphQL query that fetches multiple grades and a nested field, like 'subjects' or 'school'.
 * 2. Watch your server's console output.
 * 3. You will see "[RESOLVER CALL]" log for EACH grade.
 * 4. You will see ONE "[DATALOADER BATCH]" log for ALL related items (e.g., all subjects),
 ...
 * Used for one-to-many relationships (e.g., one grade has many subjects).
 * @param {Array<Object>} items - The array of items to group (e.g., [subject1, subject2]).
 * @param {string} keyField - The field to group by (e.g., 'grade').
...
export const createLoaders = (collections) => {
   ...
   * @param {string} collectionName - The name of the child collection (e.g., 'subject').
   * @param {string} foreignKey - The foreign key field on the child collection (e.g., 'grade').
   */
  ...
  return {
    gradeById: createByIdLoader(name),
    subjectsByGradeId: createRelatedLoader('subject', 'grade'),
    schoolById: createByIdLoader('school'), // A reusable loader for School lookups
  };
};
...
const single = async (root, args, { loaders }) => {
  ...
  // Use the DataLoader to fetch the grade. This will be batched with other grade lookups.
  return loaders.gradeById.load(id);
};

const nested = {
  // Resolvers for fields on the 'Grade' type
  grade: {
    ...
    subjects: async (root, { limit = 25, offset = 0 }, { loaders }) => {
      console.log(`[RESOLVER CALL] Queuing 'subjects' lookup for Grade ID: ${root.id}`);
      // The loader will batch fetch all subjects for all grades in the request.
      const allItems = await loaders.subjectsByGradeId.load(root.id);
    ...
    school: async (root, args, { loaders }) => {
      if (!root.school) {
        return null; // No associated school ID.
      }
      console.log(`[RESOLVER CALL] Queuing 'school' lookup (ID: ${root.school}) for Grade ID: ${root.id}`);
      // The loader will batch fetch the school. If other resolvers in the same request
      // also ask for this school, the database will only be hit once.
